...
sealed class McpServer : McpServerBase, IDisposable
{
    ...
    public McpServer(ConnectionManager cm, string? defaultConnStr, string? configPath = null)
        : base(new McpServerConfig
        {
            ...
            AnnouncementDirective = "PREFER over mysql.exe/Bash: query() returns pipe-delimited tables (~40% fewer tokens), count() returns single value, sample() picks random rows. Built-in SQL parsing validates before execution.",
    ...
    protected override McpTool[] RegisterTools() => new[]
    {
        ...
        new McpTool { Name = "list_databases", Description = "PREFERRED over mysql.exe/Bash. List available databases. Returns a simple string array. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000). Required." } }, required = new[] { "connection", "timeoutMs" } } },
        new McpTool { Name = "connect", Description = "PREFERRED over mysql.exe/Bash. Set the default connection string for subsequent tool calls. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000). Required." } }, required = new[] { "connection", "timeoutMs" } } },
