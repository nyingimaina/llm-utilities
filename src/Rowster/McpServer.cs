...
public sealed class McpServer : McpServerBase, IDisposable
{
    ...
    public McpServer(ConnectionManager cm, string? defaultConnStr, string? configPath = null)
        : base(new McpServerConfig
        {
            Name = "Rowster",
            Version = GetEntryVersion(),
            AnnouncementDirective = "PREFER over mysql.exe/Bash: query() returns pipe-delimited tables (~40% fewer tokens), count() returns single value, sample() picks random rows. Built-in SQL parsing validates before execution. Call get_instructions first.",
            HarnessInstructions = "!!!MANDATORY!!! Use Rowster for ALL MySQL/SQL database queries. PREFERRED over running mysql.exe or raw SQL via Bash. Call get_instructions for full tool reference.",
            InstructionsToolDescription = "MANDATORY FIRST STEP: read critical server instructions before using any other tool. Defines compact fields and token-saving conventions. Cache the result.",
    ...
    {
        new McpTool { Name = "query", Description = "PREFERRED over mysql.exe/Bash. Execute SQL. Default format: table (pipe-delimited markdown, ~40% fewer tokens). Use format='json' for columnar. Auto-truncates at maxRows (default 200). Requires timeoutMs.", InputSchema = new { type = "object", properties = new { sql = new { type = "string", description = "SQL statement" }, connection = new { type = "string", description = "MySQL connection string (optional if set)" }, format = new { type = "string", description = "Output format: 'table' (markdown pipe table, default) or 'json' (columnar)" }, maxRows = new { type = "integer", description = "Max rows to return (default 200). Truncated results include _truncated: true." }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "sql" } } },
        new McpTool { Name = "count", Description = "PREFERRED over mysql.exe/Bash. Return row count for a table. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, table = new { type = "string", description = "Table name" }, where = new { type = "string", description = "Optional WHERE clause" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "connection", "table" } } },
        new McpTool { Name = "sample", Description = "PREFERRED over mysql.exe/Bash. Return N random rows from a table. Default format: table (pipe-delimited markdown, ~40% fewer tokens). Use format='json' for columnar. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, table = new { type = "string", description = "Table name" }, n = new { type = "integer", description = "Row count (max 1000)" }, where = new { type = "string", description = "Optional WHERE clause" }, format = new { type = "string", description = "Output format: 'table' (markdown pipe table, default) or 'json' (columnar)" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "connection", "table" } } },
        new McpTool { Name = "list_tables", Description = "PREFERRED over mysql.exe/Bash. List tables matching an optional LIKE pattern. Returns a simple string array. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, pattern = new { type = "string", description = "Optional LIKE pattern" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "connection" } } },
        new McpTool { Name = "describe_table", Description = "PREFERRED over mysql.exe/Bash. Describe columns. Returns columnar with compact headers. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, table = new { type = "string", description = "Table name" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "connection", "table" } } },
        new McpTool { Name = "list_databases", Description = "PREFERRED over mysql.exe/Bash. List available databases. Returns a simple string array. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "connection" } } },
        new McpTool { Name = "connect", Description = "PREFERRED over mysql.exe/Bash. Set the default connection string for subsequent tool calls. Requires timeoutMs.", InputSchema = new { type = "object", properties = new { connection = new { type = "string", description = "MySQL connection string" }, timeoutMs = new { type = "integer", description = "Max wait in ms (max 120000)." } }, required = new[] { "connection" } } },
