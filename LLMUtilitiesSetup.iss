...
begin
  ...
  ChkSelectAll.OnClick := @SelectAllClick;
  ...
  ChkGemini.Left := 8;
  ChkGemini.Top := ChkCodeNavigator.Top + ChkCodeNavigator.Height + 14;
  ChkGemini.Width := ClaudePage.SurfaceWidth - 16;
  ChkGemini.Height := 17;
  ChkGemini.Caption := 'Also register with Gemini CLI (~/.gemini/settings.json)';
  ChkGemini.Checked := False;
  ...
  ChkOpenCode.Width := ClaudePage.SurfaceWidth - 16;
  ChkOpenCode.Height := 17;
  ChkOpenCode.Caption := 'Also register with OpenCode (~/.config/opencode/opencode.json)';
  ChkOpenCode.Checked := False;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ChkGemini.Checked then
          SummaryLines.Lines.Add('✓ Also registered with Gemini CLI');
        if ChkOpenCode.Checked then
          SummaryLines.Lines.Add('✓ Also registered with OpenCode');
      end
      else
      begin
        SummaryLines.Lines.Add('');
